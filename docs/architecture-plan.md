...
*Post-PoC roles (deferred)*: Developer (test impact for PRs), Product Owner (requirement coverage), Business Analyst (story validation), CI/CD Service Account (automated pipeline triggers).
...
*Deferred to post-PoC*: Analytics & Reporting dashboard, detailed Administration UI (PoC uses a simple admin settings page).
...
interface HealingPolicy {
  ...
  rules: {
    selector_update: { autoApproveThreshold: number; requireReview: boolean; allowedReviewers: Role[] };
    wait_condition: { /* same */ };
    navigation_path: { /* same */ };
    // ...
  };
